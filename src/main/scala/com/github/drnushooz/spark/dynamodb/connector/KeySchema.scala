...
import software.amazon.awssdk.services.dynamodb.model.{KeySchemaElement, KeyType}
...
private[dynamodb] object KeySchema {
  def fromDescription(keySchemaElements: Seq[KeySchemaElement]): KeySchema = {
    val hashKeyName = keySchemaElements.find(_.keyType == KeyType.HASH).get.attributeName
    val rangeKeyName = keySchemaElements.find(_.keyType == KeyType.RANGE).map(_.attributeName)
