...
private[dynamodb] case class ColumnSchema(keySchema: KeySchema, sparkSchema: StructType) {
  private type Attr = (String, Int, DataType)
  ...
  private val attributeIndices = columnNames.zipWithIndex
    ...
    .map { case (name, index) =>
      (name, index, sparkSchema(name).dataType)
    }

  def keys(): Either[Attr, (Attr, Attr)] = {
    keyIndices
  }

  def attributes(): Seq[Attr] = {
    attributeIndices
  }
}
